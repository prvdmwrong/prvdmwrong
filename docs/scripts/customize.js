...
try {
	function monospaceFont() {
		const MONOSPACE_FONTS = ["JetBrains Mono", "Source Code Pro", "Fira Code", "DM Mono", "Space Mono"]
		...
		for (const font of MONOSPACE_FONTS) {
			...
			preloadLink.setAttribute(
				"href",
				"https://fonts.googleapis.com/css?family=" + font.replaceAll(" ", "+") + "&display=fallback"
			)
	...
	function codeFor() {
		const CODE_LANGUAGES = ["Luau", "TypeScript"]
		...
		for (const language of CODE_LANGUAGES) {
			...
			document.querySelectorAll(".only-" + value).forEach(element => elements.push(element))
